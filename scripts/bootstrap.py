...
def run_migration(skill_dir: Path, runtime_home: Path, hermes: bool = False) -> None:
    ...
    **每一次**升级都还会重新联网读一遍配了 key_tiebreakers 的台账——
    ...
    if recovery.returncode == 0:
        # 🔴 0 本身有歧义，外部审查指出：它既可能是"状态从头到尾没被碰过"
        #    （多半败在联网取数那一步，走不到写状态那里——安全回滚），
        #    也可能是"已经完整写完 + 清理完事务日志，但在那之后（比如
        #    进程即将正常退出前）被强杀"——这种情况下事务日志同样已经
        #    没了，pending_state_transaction() 一样返回 None，跟"从没
        #    开始"在这个退出码上长得一模一样。仅凭退出码 0 分不出这两种，
        #    必须多看一眼迁移完成标记有没有落盘——它是在事务真正提交时
        #    跟其余状态文件一起原子写入的（见 migrate_rc15_key_state.py
        #    的 final[MIGRATION_MARKER_FILE]），标记在就说明那次改写
        #    真的完整落地过，回滚代码反而会制造"状态新、代码旧"。
        if _migration_already_done(runtime_home):
            print("ℹ️ 迁移子进程虽然异常退出，但完成标记已经落盘——"
                  "状态其实已经完整写完（多半是清理或退出阶段被打断）。"
                  "保留新代码，不回滚。")
            return
        raise BootstrapError(
    ...
    if recovery.returncode == 3:
        # 确认没写完，已经照备份整体复原——"状态回到了旧代码认得的
        # 样子"，可以放心让下面的异常触发回滚。这条不用查完成标记：
        # 复原动作本身已经把状态改回旧样子，标记要么从没写过，
        # 要么就算写过也已经被这次复原一并抹掉，不存在"标记在但状态
        # 是旧的"这种撞车。
        raise BootstrapError(
            f"升级后的状态迁移未完成（退出码 {result.returncode}），"
            f"状态已确认回到升级前的样子，正在回滚代码。"
