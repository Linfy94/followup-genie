...
ORGANIZATION_RE = re.compile(
    # 🔴 2026-08-21 复审发现：原来只认「2-60 个连续中文字符 + 后缀」，
    # "ABC有限公司" 这种纯英文缩写紧跟中文后缀的写法完全漏检——前缀不含
    # 一个中文字符，\u3400-\u9fff 那段匹配不上，正则直接跳过整段。
    #
    # 补一条英文/数字前缀分支：只认大写字母 + 数字（1-20 位，允许
    # . & - 这类公司名常见标点），不接受小写字母。这是刻意的取舍：
    # 真实企业英文缩写几乎总是全大写（ABC / IBM / 3M），第一版试过接受
    # 大小写混合并允许内部空格，结果连 "...variable_name and集团..."
    # 这种代码注释里纯属巧合的英文单词紧跟中文后缀词也会被命中——
    # 只认大写把「一段普通英文散文」和「一个缩写」分开，仍覆盖目标场景。
    #
    # 两条分支各自独立匹配，findall 仍然只在真的紧邻后缀词的地方命中——
    # "3M中国有限公司" 这类中英混合不受影响，因为 "3M" 后面不是直接跟
    # 后缀词，只有「中国有限公司」这段满足中文分支。
    r"(?:[\u3400-\u9fff]{2,60}|[A-Z0-9][A-Z0-9.&-]{0,20})"
