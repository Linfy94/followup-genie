...
ORGANIZATION_RE = re.compile(
    # 🔴 2026-08-21 复审两轮发现：
    # 第一版只认「2-60 个连续中文字符 + 后缀」，纯英文缩写紧跟中文后缀
    # （"ABC有限公司"）完全漏检。补了英文前缀分支，但**只认大写**——
    # 这是为了排除 "...variable_name and集团..." 这种代码注释里巧合的
    # 英文单词紧跟中文后缀词的误报。
    #
    # 第二版复审指出："iRobot有限公司"、"abc有限公司" 这类大小写混合/
    # 全小写的真实缩写写法，同样会被"只认大写"漏掉——而业务的态度很
    # 明确：**宁可构建失败后人工确认，也不能放过真实客户名**。这条
    # 优先级排在"避免误报"之上，所以这次改成不限制大小写。
    #
    # 代价：像 "and集团" 这类误报会重新出现（"and" 和 "abc" 在字符
    # 特征上完全一样，正则分不出哪个更像公司名缩写，这是纯文本匹配的
    # 理论极限，不借助语义/字典无法两全）。真触发了，处理成本很低——
    # 加进 `_manifest.ALLOWED_EXAMPLE_ORGANIZATIONS` 白名单，或者把
    # 那句注释/文档改个说法，比漏掉一个真实客户名的代价小得多。
    #
    # 仍保留「不含空格」这一条：挡住的是"一整段英文散文 + 巧合词"这种
    # 更离谱的超长误报，跟"能不能识别 abc/iRobot 这类紧凑缩写"无关，
    # 不冲突。
    #
    # 两条分支各自独立匹配，findall 仍然只在真的紧邻后缀词的地方命中——
    # "3M中国有限公司" 这类中英混合不受影响，因为 "3M" 后面不是直接跟
    # 后缀词，只有「中国有限公司」这段满足中文分支。
    #
    # 🔴 第三版复审发现：带地区括号的企业全称（"XX进出口贸易（宁波）
    # 有限公司"、"XX贸易(上海)有限公司"）完全漏检——括号本身不在
    # \u3400-\u9fff 范围内，会打断"紧邻后缀词"这条连续字符要求，
    # 真实台账里这类写法很常见。补一段可选的"括号+地区名"，允许
    # 紧贴在前缀和后缀之间（两侧最多各一个空格，兼容"XX (深圳) 有限
    # 公司"这类带空格的写法），没有括号时行为不变。
    r"(?:[\u3400-\u9fff]{2,60}|[A-Za-z0-9][A-Za-z0-9.&-]{0,20})"
    r"(?:\s?[（(][\u3400-\u9fffA-Za-z0-9]{1,20}[）)]\s?)?"
    r"(?:有限责任公司|股份有限公司|有限公司|集团|医院|酒店|合作社)"
)
